...
from services.interoperation import interoperationService
...
class Stack(Stack):
    def __init__(self, scope: Construct, **kwargs) -> None:
        ...
        os_cidr_ranges = [
            cidr.strip()
            for cidr in config["main"]["os_cidr_ranges"].split(",")
        ]
        ...
        if(config['cloudfront']['deploy'] == "true"):
            ...
            public_key_id = config["cloudfront"].get("public_key_id")
            cf_domain = config["cloudfront"].get("cf_domain")
            
        self.secret = secretsmanager.Secret(self, "Secret",
            secret_name="{}/{}".format(config['main']['secret_prefix'], config['main']['tier']),
            secret_object_value={
                ...
                "memgraph_user": SecretValue.unsafe_plain_text(config['db']['memgraph_user']),
                "memgraph_password": SecretValue.unsafe_plain_text(config['db']['memgraph_password']),
...
#            prefix="alb-logs/"
        ...
        # Extracts Subnets
        #subnet_efs1 = config.get('Subnets', 'subnet_efs1')
        #subnet_efs2 = config.get('Subnets', 'subnet_efs2')
        #subnets_efs = ec2.SubnetSelection(
            #subnets=[
                #ec2.Subnet.from_subnet_id(self, "Subnet_efs1", subnet_efs1),
                #ec2.Subnet.from_subnet_id(self, "Subnet_efs2", subnet_efs2)
            #]
        #)
        ...
        #memgraphService().createService(self, config)
        # Memgraph Service
        memgraph.memgraphService.createService(self, config)
        frontendService().createService(self, config, [app_security_group])
        backendService().createService(self, config, [app_security_group])
        interoperationService().createService(self, config, [app_security_group])
        # Memgraph Service
        #memgraph.memgraphService.createService(self, config)
